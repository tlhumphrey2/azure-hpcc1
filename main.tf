...
resource "azurerm_subnet" "subnet" {
  name                 = "${var.prefix_cluster_name}subnet"
...
resource "azurerm_network_interface" "tf-guide-nic" {
  name                      = "${var.prefix_cluster_name}tf-guide-nic"
  location                  = "${var.location}"
  resource_group_name       = "${azurerm_resource_group.tf_azure_guide.name}"
...
resource "azurerm_public_ip" "tf-guide-pip" {
  name                         = "${var.prefix_cluster_name}-ip"
  location                     = "${var.location}"
  resource_group_name          = "${azurerm_resource_group.tf_azure_guide.name}"
...
resource "azurerm_virtual_machine" "cluster-node" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "echo ${var.admin_password} | sudo -S ls -l;sudo su -",
      "exec 3>&1 4>&2",
